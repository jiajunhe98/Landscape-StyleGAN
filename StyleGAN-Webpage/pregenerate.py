...
def save_tensor_images(random_n, image_tensor, size, num_images, nrow, time, detail):
    ...
    plt.margins(0, 0)
    fig.savefig("./plots/save"+str(num_images)+str(time)+str(detail)+"_"+str(random_n)+".jpg", dpi=100, pad_inches=0)
    plt.close()


def generate(random_n, num, nrow, time, detail):
    ...
    with torch.no_grad():
        ...
        save_tensor_images(random_n, images, (3, images.shape[2], images.shape[2]), num, nrow, time, detail)
...
for random_n in range(15, 30):
    for time in [0, 1, 4]:
        if time == 4:
            generate(random_n, 1, 1, time, 4)
            generate(random_n, 2, 2, time, 4)
            generate(random_n, 3, 3, time, 4)
            ...
            for detail in [0, 1, 4]:
                ...
                generate(random_n, 2, 2, time, detail)
                generate(random_n, 3, 3, time, detail)
